...
Player::Player() : MovingEntity(100, 1), touchBegan(ccp(0,0)), mNextTouchedDirection(ccp(0,0)), mStreakPlayer(NULL), 
	winner(false)
{
	this->mEntitySprite = cocos2d::CCSprite::create("1_.png");
...
void Player::Init()
{
	...
	//move effect motion streak
	mStreakPlayer = CCMotionStreak::create(.35f, 1 , mEntitySprite->getContentSize().width,
		ccc3(255,144,0), "playerStreak.png");
	CCNode* parent = mEntitySprite->getParent();
	parent->addChild(mStreakPlayer, 1);
...
bool Player::ccTouchBegan(CCTouch* touch, CCEvent* event)
{
	...
	this->addMotionStreakPoint(touchBegan);
...
void Player::ccTouchMoved(CCTouch* touch, CCEvent* event)
{
	...
	CCPoint moveVec;
		...
		//touch  effect
	this->addMotionStreakPoint(touchEnd);
...
// Touch Effect
void Player::resetMotionStreak()
{
	this->removeChildByTag(MOTION_STREAK_TAG, true);
	//CCMotionStreak* streak = CCMotionStreak::create(0.5f, 1, 20, ccc3(0,255,0), "TouchEffect.png");
	CCMotionStreak* streak = CCMotionStreak::create(0.5f, 1, 20, ccc3(255,144,0), "TouchEffect.png");

	CCNode* parent = mEntitySprite->getParent();
	parent->addChild(streak, 10, MOTION_STREAK_TAG);
}

void Player::addMotionStreakPoint(CCPoint point)
{
	CCNode* parent = mEntitySprite->getParent();
	CCMotionStreak *streak =  (CCMotionStreak*) parent->getChildByTag(MOTION_STREAK_TAG);
...
{
	//Check the win
	if(PathPlanner::GetInstance()->GetLevelMap()->IsInChipTiles(this->GetSpriteTile()))
	{
		this->winner = true;
		PathPlanner::GetInstance()->GetLevelMap()->SetPlayerWinner(true);
		return true;
	}

	bool gameOver = MoveToDirection(mNextTouchedDirection, dt);

	mStreakPlayer->setPosition(this->mEntitySprite->getPosition());

	//GEt the item in tile
	if(PathPlanner::GetInstance()->GetLevelMap()->GetItemInTile(this->GetSpriteTile()) == CHUNK )
	{
		PathPlanner::GetInstance()->GetLevelMap()->GetAIEffect()->DecreaseOpacityLevel();
		PathPlanner::GetInstance()->GetLevelMap()->DecreaseChunkNum();
	}
...
bool Player::MoveToDirection(cocos2d::CCPoint nextDirection, float dt)
{
	//helper
