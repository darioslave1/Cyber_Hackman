...
GameLevel::GameLevel(void): mStopGame(false), mExitLevel(false), mDoorSprite(false), mDoorScale(0.2f)
{
}
...
bool GameLevel::init()
{
	...
	//	CCSprite* AIEffect = CCSprite::create("AI-Effect_1.png");
	//	AIEffect->setPosition( ccp(visibleSize.width/2 , visibleSize.height/2 ));
	//	this->addChild( AIEffect );

		mDoorSprite = CCSprite::create("Door.png");
		mDoorSprite->setVisible(false);
		mDoorSprite->setScale(0.1);
		mDoorSprite->setPosition(PathPlanner::GetInstance()->GetLevelMap()->positionForTileCoord(ccp(12,8)));
		this->addChild(mDoorSprite);
...
void GameLevel::Update(float dt)
{
	...
	if(mStopGame)
	{
		if(mExitLevel == false)
		{
			...
			//WINNER! Create the new Scene
			if(PathPlanner::GetInstance()->GetLevelMap()->IsPlayerWinner())
			{
				WinnerScene *winnerScene = WinnerScene::create();
				mExitLevel = true;

				CCDirector::sharedDirector()->replaceScene(CCTransitionProgressInOut::create(0.8f, winnerScene) );
	...
	else
	{
		if(mLevelMap->GetChunkNum() <= 0)
		{
			...
			mDoorSprite->setVisible(true);
			mDoorSprite->setRotation(mDoorSprite->getRotation() + 20);

			mDoorSprite->setScale(mDoorSprite->getScale() + mDoorScale);

			if(mDoorSprite->getScale() > 2.f)
				mDoorScale = -0.02;
			if(mDoorSprite->getScale() <= 1.f)
				mDoorScale = 0.02;
